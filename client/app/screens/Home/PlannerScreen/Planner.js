...
export default function Planner() {
	...
	useEffect(() => {
		if (lessons && selectedDateWeekDates && lessons.length > 0)
			updateWeekPlan();
				...
				{selectedDateWeekDates.map((weekDay, index) => {
					if (weekDay.dayName === daysOfWeek[selectedDate.getDay()]) {
						return (
							...
							>
								<Text style={[PlannerStyles.txtDay, { color: "#000" }]}>
						...
						return (
							...
							>
								<Text style={PlannerStyles.txtDay}>
									{weekDay.dayName.slice(0, 3)}
								</Text>
							</TouchableOpacity>
						);
	...
	const createList = () => {
		return (
			<View style={PlannerStyles.titleColContainer}>
				<DataTable>
					<DataTable.Header style={{ backgroundColor: "#FBFF12" }}>
						<DataTable.Title
							textStyle={[ManageStyles.tableTxtTitleStyle, colorTxt]}
						>
							Event
						</DataTable.Title>
				...
				{userData.role !== "Student" ? (
					<View>
						...
						>
							{showNewEvent ? (
								<NewEventModal
									closeModal={() => setShowNewEvent(!showNewEvent)}
								/>
							) : (
								<></>
							)}
							...
							<Text style={[styles.loginButtonText, { fontSize: 15 }]}>
								Add event
							</Text>
						</TouchableOpacity>
					</View>
